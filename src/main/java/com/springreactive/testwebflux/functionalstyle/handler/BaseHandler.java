...
public class BaseHandler {
    public <T> Mono<ServerResponse> apiResponseSuccess(Mono<T> mono) {
        return ServerResponse
                .ok()
                .body(mono.map(APIResponse::success).defaultIfEmpty(APIResponse.defaultz()).log(), APIResponse.class);
    }

    public <T> Mono<ServerResponse> apiResponseSuccess(Flux<T> flux) {
        return ServerResponse
                .ok()
                .body(flux.collectList().map(APIResponse::success).defaultIfEmpty(APIResponse.defaultz()).log(), APIResponse.class);
    }
    ...
    public <T> Mono<ServerResponse> apiFluxResponseSuccess(Flux<T> flux, MediaType mediaType) {
        return ServerResponse
                ...
                .body(flux.map(APIResponse::success).defaultIfEmpty(APIResponse.defaultz()).log(), APIResponse.class);
    }
}
